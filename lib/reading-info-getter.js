        ...
        for (const message of conversation.messages) {
          ...
          if (message.sender.address === 'busybee@blogtrottr.com') {
            ...
            $('table.btrcontent').each(function() {
              ...
              const title = he.decode(el.find('a').first().text().trim());
              const description = he.decode(el.find('div').text().trim()) || '';
              links.push({
                title,
                url,
                description,
          ...
          } else if (message.sender.address === 'info@renaissanceperiodization.com') {
            if (/save up to/i.test(message.subject)) {
              parsed = true;
            }
          } else if (message.sender.address === 'updates@academia-mail.com') {
            const link = $( 'a[href*="/resource/work/*email_work_card=title"]').first();
            if (link) {
            ...
            if (link) {
              const url = link.attr('href');
              const title = he.decode(link.text().trim());
              const description = he.decode($(link).parent().text().trim());
              parsed = true;
              await this.#onReading([{
                url,
                title,
